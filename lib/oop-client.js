...
function coreAlgo(coverageLevel, dedIndRem, oopIndRem, dedFamRem, oopFamRem, serviceCost, benefitType, benefitValue, service, previousCost) {
  ...
  if (dedIndRem < previousCost) {
    ...
    dedIndRem = dedIndRem - previousCost;
  }

  if (oopIndRem < previousCost) {
    oopIndRem = 0;
  } else {
    oopIndRem = oopIndRem - previousCost;
  }

  if (dedFamRem < previousCost) {
    dedFamRem = 0;
  } else {
    dedFamRem = dedFamRem - previousCost;
  }

  if (oopFamRem < previousCost) {
    oopFamRem = 0;
  } else {
    oopFamRem = oopFamRem - previousCost;
  }
  ...
  if (benefitType === 'copay_ad') {
    if (dedRemaining === 0) { // which means _both_ ind and fam deductibles are 0, so benefits kick in
      ...
      return ({ oop: dedRemaining, breakdown: `For ${service}, your OOP cost is your remaining ${controllingDedType} deductible $${dedRemaining.toFixed(2)}` });
...
/*    getEpisodeData(episode, category).then((result) => {
    ...
    getEpisodeData(episode, category).then((result) => {
      ...
      return runCalc(episodeData[1]._id, costData.allowedAmt);
    }).then((costData) => {
      console.log('costData', costData);
      costTable.push(costData);
      let totalLow = +costTable[0].allowedAmt[0].oop + +costTable[1].allowedAmt[0].oop;
      let totalAverage = +costTable[0].allowedAmt[1].oop + +costTable[1].allowedAmt[1].oop;
      let totalHigh = +costTable[0].allowedAmt[2].oop + +costTable[1].allowedAmt[2].oop;
      let total = {
        totals: [
          {
            label: 'Low',
