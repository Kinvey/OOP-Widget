    ...
    getEpisodeData(serviceId, episode, category).then((result) => {
      ...
      .then((costData) => {
        ...
        costData.allowedAmt.forEach((facility) => {
          const narrative2 = `After subtracting $${facility.oop}, your assumed remaining deductible is $${Math.max(0, costData.dedIndRem - facility.oop).toFixed(2)} and assumed remaining out of pocket maximum is $${Math.max(0, costData.oopIndRem - facility.oop).toFixed(2)}.`;
