...
function coreAlgo(coverageLevel, dedIndRem, oopIndRem, dedFamRem, oopFamRem, dedIndLimit, oopFamLimit, serviceCost, benefitType, benefitValue, service, previousCost) {
  ...
  if (benefitType.toLowerCase() === 'copayad') {
    if (dedRemaining === 0) { // which means _both_ ind and fam deductibles are 0, so benefits kick in
      oop = benefitValue;
      breakdown = `For ${service}, your deductible has been met and you will pay your copay of $${oop.toFixed(2)}.`;
      ...
      oop = dedRemaining;
      breakdown = `For ${service}, your remaining deductible is less than the contracted rate of $${serviceCost}, you will pay your remaining deductible of $${oop.toFixed(2)}.`;
    } else if (serviceCost > oopRemaining) { // serviceCost<=dedRemaining
      oop = oopRemaining;
      breakdown = `For ${service}, you will meet your out of pocket maximum. You will pay your remaining out of pocket of $${oop.toFixed(2)}.`;
    } else {
      oop = serviceCost;
  ...
  if (benefitType.toLowerCase() === 'coins') {
    ...
    if (dedRemaining === 0) { // which means _both_ ind and fam deductibles are 0, so benefits kick in
      ...
      if (oop > oopRemaining) {
        ...
        breakdown = `For ${service}, your deductible has been met and you owe ${coinsReadable}% of the $${serviceCost} contracted rate. You will have to pay $${oop.toFixed(2)} ( $${serviceCost} x ${coinsReadable}% ) = $${oop.toFixed(2)}.`;
      }
      ...
      if (oop > oopRemaining) {
        ...
        breakdown = `For ${service}, your remaining deductible of $${dedRemaining.toFixed(2)} is less than the contracted rate of $${serviceCost}. You owe your remaining deductible of $${dedRemaining.toFixed(2)} plus ${coinsReadable}% of the $${(serviceCost - dedRemaining).toFixed(2)} remaining contracted rate. You will pay $${oop.toFixed(2)} [ $${dedRemaining.toFixed(2)} + ( $${(serviceCost - dedRemaining).toFixed(2)} x ${coinsReadable}% = $${oop.toFixed(2)} ].`;
      }
    ...
    if (serviceCost > oopRemaining) {
      ...
      oop = serviceCost;
      breakdown = `For ${service}, you will have to pay the contracted rate of $${oop.toFixed(2)}, as your deductible will not be met.`;
    } 
  }
  ...
  if (benefitType.toLowerCase() === 'percent') {
    ...
    oop = serviceCost * coinsFraction;
    breakdown = `For ${service}, you owe ${coinsReadable}% of the $${serviceCost} contracted rate. You will have to pay $${oop.toFixed(2)} ( $${serviceCost} x ${coinsReadable}% ) = $${oop.toFixed(2)}.`;
    if (oop > oopRemaining) {
      oop = oopRemaining;
      breakdown = `For ${service}, you will meet your out of pocket maximum. You will pay your remaining out of pocket of $${oop.toFixed(2)}.`;
    } 
...
const calculateOop = (query, modules) => {
  return new Promise((resolve, reject) => {
    ...
    const payerName = JSON.parse(query).trading_partner_id;
    ...
    getEpisodeData(serviceId, episode, category).then((result) => {
      ...
      .then((costData) => {
        ...
        costData.allowedAmt.forEach((facility) => {
          let narrative2;
          if (dedFamLimit > 0) {
            narrative2 = `After subtracting $${facility.oop}, your assumed remaining deductible is $${Math.max(0, costData.dedIndRem - facility.oop).toFixed(2)} and assumed remaining out of pocket maximum is $${Math.max(0, costData.oopIndRem - facility.oop).toFixed(2)}.`;
          } else {
            narrative2 = `After subtracting $${facility.oop}, your assumed remaining out of pocket maximum is $${Math.max(0, costData.oopIndRem - facility.oop).toFixed(2)}.`;
          }
      ...
      .then((costData) => {
        ...
        return (episodeData[2]) ? runCalc(episodeData[2]._id, costData.allowedAmt) : costData;
      })
      .then((costData) => {
        console.log('costData', costData);
        costData.allowedAmt.forEach((facility) => {
        ...
        if (costTable[1] === undefined) {
          ...
          narrative1 = `Your total out of pocket cost estimate: $${costTable[0].allowedAmt[0].oop} + $${costTable[1].allowedAmt[0].oop} = $${totalLow.toFixed(2)}.`;
          totalAverage = +costTable[0].allowedAmt[1].oop + +costTable[1].allowedAmt[1].oop;
          narrative2 = `Your total out of pocket cost estimate: $${costTable[0].allowedAmt[0].oop} + $${costTable[1].allowedAmt[1].oop} = $${totalAverage.toFixed(2)}.`;
          totalHigh = +costTable[0].allowedAmt[2].oop + +costTable[1].allowedAmt[2].oop;
          narrative3 = `Your total out of pocket cost estimate: $${costTable[0].allowedAmt[0].oop} + $${costTable[1].allowedAmt[2].oop} = $${totalHigh.toFixed(2)}.`;
        } else {
          totalLow = +costTable[0].allowedAmt[0].oop + +costTable[1].allowedAmt[0].oop + +costTable[2].allowedAmt[0].oop;
          narrative1 = `Your total out of pocket cost estimate: $${costTable[0].allowedAmt[0].oop} + $${costTable[1].allowedAmt[0].oop} + $${costTable[2].allowedAmt[0].oop} = $${totalLow.toFixed(2)}.`;
          totalAverage = +costTable[0].allowedAmt[1].oop + +costTable[1].allowedAmt[1].oop + +costTable[2].allowedAmt[1].oop;
          narrative2 = `Your total out of pocket cost estimate: $${costTable[0].allowedAmt[0].oop} + $${costTable[1].allowedAmt[1].oop} + $${costTable[2].allowedAmt[1].oop} = $${totalAverage.toFixed(2)}.`;
          totalHigh = +costTable[0].allowedAmt[2].oop + +costTable[1].allowedAmt[2].oop + +costTable[2].allowedAmt[2].oop;
          narrative3 = `Your total out of pocket cost estimate: $${costTable[0].allowedAmt[0].oop} + $${costTable[1].allowedAmt[2].oop} + $${costTable[2].allowedAmt[2].oop} = $${totalHigh.toFixed(2)}.`;
        }
